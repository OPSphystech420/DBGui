...
void CTextEditor::SelectNextOccurrenceOf(const char* aText, int aTextSize, bool aCaseSensitive)
{
	ClearSelections();
	ClearExtraCursors();
...
void CTextEditor::SelectAllOccurrencesOf(const char* aText, int aTextSize, bool aCaseSensitive)
{
	ClearSelections();
	ClearExtraCursors();
	SelectNextOccurrenceOf(aText, aTextSize, -1, aCaseSensitive);
...
void CTextEditor::SetCursorPosition(int aLine, int aCharIndex)
{
...
bool CTextEditor::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool bBackground, bool bBorder)
{
	...
	ImGui::PushStyleColor(ImGuiCol_ChildBg, bBackground ? ImGui::ColorConvertU32ToFloat4(m_Palette[(int)PaletteIndex::Background]) : ImVec4());
	ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0.0f, 0.0f));
	{
		ImGui::BeginChild(aTitle, aSize, bBorder ? ImGuiChildFlags_Border : ImGuiChildFlags_None, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoNavInputs);
		{
...
{
	for (int i = m_Operations.size() - 1; i > -1; i--)
	{
...
std::string CTextEditor::GetText(const Coordinates& aStart, const Coordinates& aEnd) const
{
	...
	std::string result;
	auto lstart = aStart.m_iLine;
	auto lend = aEnd.m_iLine;
...
std::string CTextEditor::GetSelectedText(int aCursor) const
{
	if (aCursor == -1)
		aCursor = m_State.m_iCurrentCursor;

	return GetText(m_State.m_Cursors[aCursor].GetSelectionStart(), m_State.m_Cursors[aCursor].GetSelectionEnd());
}

void CTextEditor::SetCursorPosition(const Coordinates& aPosition, int aCursor, bool aClearSelection)
{
	if (aCursor == -1)
		aCursor = m_State.m_iCurrentCursor;
	...
	if (aClearSelection)
		m_State.m_Cursors[aCursor].m_InteractiveStart = aPosition;
	if (m_State.m_Cursors[aCursor].m_InteractiveEnd != aPosition)
	{
		m_State.m_Cursors[aCursor].m_InteractiveEnd = aPosition;
		EnsureCursorVisible();
	}
}
...
int CTextEditor::InsertTextAt(Coordinates& /* inout */ aWhere, const char* aValue)
{
	...
	while (*aValue != '\0')
	{
		...
		{
			if (cindex < (int)m_Lines[aWhere.m_iLine].size())
			{
				auto& newLine = InsertLine(aWhere.m_iLine + 1);
				auto& line = m_Lines[aWhere.m_iLine];
				AddGlyphsToLine(aWhere.m_iLine + 1, 0, line.begin() + cindex, line.end());
				RemoveGlyphsFromLine(aWhere.m_iLine, cindex);
			}
			else
			{
				InsertLine(aWhere.m_iLine + 1);
			}
			++aWhere.m_iLine;
			aWhere.m_iColumn = 0;
			cindex = 0;
		...
		else
		{
			...
			while (d-- > 0 && *aValue != '\0')
				AddGlyphToLine(aWhere.m_iLine, cindex++, Glyph(*aValue++, PaletteIndex::Default));
			aWhere.m_iColumn = GetCharacterColumn(aWhere.m_iLine, cindex);
		}
	}
...
void CTextEditor::InsertTextAtCursor(const char* aValue, int aCursor)
{
	if (aValue == nullptr)
		return;
	if (aCursor == -1)
	...
	auto start = std::min(pos, m_State.m_Cursors[aCursor].GetSelectionStart());
...
bool CTextEditor::Move(int& aLine, int& aCharIndex, bool aLeft, bool aLockLine) const
{
	...
	if (aLeft)
	{
		if (aCharIndex == 0)
		{
			if (aLockLine || aLine == 0)
				return false;
			aLine--;
			aCharIndex = m_Lines[aLine].size();
		}
		else
		{
			aCharIndex--;
			while (aCharIndex > 0 && IsUTFSequence(m_Lines[aLine][aCharIndex].m_cChar))
				aCharIndex--;
		}
	}
	else // right
	{
		if (aCharIndex == m_Lines[aLine].size())
		{
			if (aLockLine || aLine == m_Lines.size() - 1)
				return false;
			aLine++;
			aCharIndex = 0;
		}
		...
		{
			int seqLength = UTF8CharLength(m_Lines[aLine][aCharIndex].m_cChar);
			aCharIndex = std::min(aCharIndex + seqLength, (int)m_Lines[aLine].size());
		}
	}
	return true;
}

void CTextEditor::MoveCharIndexAndColumn(int aLine, int& aCharIndex, int& aColumn) const
{
	assert(aLine < m_Lines.size());
	assert(aCharIndex < m_Lines[aLine].size());
	char c = m_Lines[aLine][aCharIndex].m_cChar;
	aCharIndex += UTF8CharLength(c);
	if (c == '\t')
		aColumn = (aColumn / m_iTabSize) * m_iTabSize + m_iTabSize;
	else
		aColumn++;
}

void CTextEditor::MoveCoords(Coordinates& aCoords, MoveDirection aDirection, bool aWordMode, int aLineCount) const
{
	int charIndex = GetCharacterIndexR(aCoords);
	int lineIndex = aCoords.m_iLine;
	switch (aDirection)
	{
	case MoveDirection::Right:
		if (charIndex >= m_Lines[lineIndex].size())
		{
			...
			{
				aCoords.m_iLine = std::max(0, std::min((int)m_Lines.size() - 1, lineIndex + 1));
				aCoords.m_iColumn = 0;
			}
		}
		else
		{
			...
			if (aWordMode)
			{
				...
				aCoords.m_iColumn = std::max(aCoords.m_iColumn, oneStepRightColumn);
			}
			else
				aCoords.m_iColumn = oneStepRightColumn;
		}
		break;
	case MoveDirection::Left:
		if (charIndex == 0)
		{
			if (lineIndex > 0)
			{
				aCoords.m_iLine = lineIndex - 1;
				aCoords.m_iColumn = GetLineMaxColumn(aCoords.m_iLine);
			}
		}
		else
		{
			Move(lineIndex, charIndex, true);
			aCoords.m_iColumn = GetCharacterColumn(lineIndex, charIndex);
			if (aWordMode)
				aCoords = FindWordStart(aCoords);
		}
		break;
	case MoveDirection::Up:
		aCoords.m_iLine = std::max(0, lineIndex - aLineCount);
		break;
	case MoveDirection::Down:
		aCoords.m_iLine = std::max(0, std::min((int)m_Lines.size() - 1, lineIndex + aLineCount));
		break;
	}
}
...
void CTextEditor::MoveLeft(bool aSelect, bool aWordMode)
{
	if (m_Lines.empty())
		return;
...
void CTextEditor::MoveRight(bool aSelect, bool aWordMode)
{
	if (m_Lines.empty())
		return;
...
{
	SetCursorPosition(Coordinates(0, 0), m_State.m_iCurrentCursor, !aSelect);
}
...
void CTextEditor::CTextEditor::MoveBottom(bool aSelect)
{
	...
	SetCursorPosition(newPos, m_State.m_iCurrentCursor, !aSelect);
}

void CTextEditor::MoveHome(bool aSelect)
{
	for (int c = 0; c <= m_State.m_iCurrentCursor; c++)
		SetCursorPosition(Coordinates(m_State.m_Cursors[c].m_InteractiveEnd.m_iLine, 0), c, !aSelect);
}
...
void CTextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	for (int c = m_State.m_iCurrentCursor; c > -1; c--) // order important here for typing \n in the same line at the same time
	{
		...
		if (aChar == '\n')
		{
			...
			AddGlyphsToLine(coord.m_iLine + 1, newLine.size(), line.begin() + cindex, line.end());
...
void CTextEditor::Backspace(bool aWordMode)
{
	assert(!m_bReadOnly);
...
void CTextEditor::Delete(bool aWordMode, const EditorState* aEditorState)
{
	assert(!m_bReadOnly);
	...
	{
		UndoRecord u;
		u.m_Before = aEditorState == nullptr ? m_State : *aEditorState;
		for (int c = m_State.m_iCurrentCursor; c > -1; c--)
		{
			if (!m_State.m_Cursors[c].HasSelection())
				continue;
...
void CTextEditor::SetSelection(Coordinates aStart, Coordinates aEnd, int aCursor)
{
	if (aCursor == -1)
		aCursor = m_State.m_iCurrentCursor;
	...
	m_State.m_Cursors[aCursor].m_InteractiveStart = aStart;
	SetCursorPosition(aEnd, aCursor, false);
}

void CTextEditor::SetSelection(int aStartLine, int aStartChar, int aEndLine, int aEndChar, int aCursor)
{
...
bool CTextEditor::FindMatchingBracket(int aLine, int aCharIndex, Coordinates& out)
{
	if (aLine > m_Lines.size() - 1)
		return false;
	int maxCharIndex = m_Lines[aLine].size() - 1;
...
void CTextEditor::EnsureCursorVisible(int aCursor, bool aStartToo)
{
	if (aCursor == -1)
		aCursor = m_State.GetLastAddedCursorIndex();

	m_iEnsureCursorVisible = aCursor;
	m_bEnsureCursorVisibleStartToo = aStartToo;
	return;
}
...
CTextEditor::Coordinates CTextEditor::GetActualCursorCoordinates(int aCursor, bool aStart) const
{
	if (aCursor == -1)
		return SanitizeCoordinates(aStart ? m_State.m_Cursors[m_State.m_iCurrentCursor].m_InteractiveStart : m_State.m_Cursors[m_State.m_iCurrentCursor].m_InteractiveEnd);
	else
		return SanitizeCoordinates(aStart ? m_State.m_Cursors[aCursor].m_InteractiveStart : m_State.m_Cursors[aCursor].m_InteractiveEnd);
}
...
int CTextEditor::GetCharacterIndexL(const Coordinates& aCoords) const
{
	if (aCoords.m_iLine >= m_Lines.size())
		return -1;

	auto& line = m_Lines[aCoords.m_iLine];
	int c = 0;
	int i = 0;
	int tabCoordsLeft = 0;

	for (; i < line.size() && c < aCoords.m_iColumn;)
	{
		if (line[i].m_cChar == '\t')
		{
			if (tabCoordsLeft == 0)
...
int CTextEditor::GetCharacterIndexR(const Coordinates& aCoords) const
{
	if (aCoords.m_iLine >= m_Lines.size())
		return -1;
	int c = 0;
	int i = 0;
	for (; i < m_Lines[aCoords.m_iLine].size() && c < aCoords.m_iColumn;)
		MoveCharIndexAndColumn(aCoords.m_iLine, i, c);
	return i;
}
...
void CTextEditor::RemoveLines(int aStart, int aEnd)
{
	assert(!m_bReadOnly);
	assert(aEnd >= aStart);
	...
	for (int c = 0; c <= m_State.m_iCurrentCursor; c++)
	{
		if (m_State.m_Cursors[c].m_InteractiveEnd.m_iLine >= aStart)
		{
			int targetLine = m_State.m_Cursors[c].m_InteractiveEnd.m_iLine - (aEnd - aStart);
			targetLine = targetLine < 0 ? 0 : targetLine;
			m_State.m_Cursors[c].m_InteractiveEnd.m_iLine = targetLine;
		}
		if (m_State.m_Cursors[c].m_InteractiveStart.m_iLine >= aStart)
		{
			int targetLine = m_State.m_Cursors[c].m_InteractiveStart.m_iLine - (aEnd - aStart);
			targetLine = targetLine < 0 ? 0 : targetLine;
...
void CTextEditor::DeleteRange(const Coordinates& aStart, const Coordinates& aEnd)
{
	...
	if (aStart.m_iLine == aEnd.m_iLine)
	{
		auto n = GetLineMaxColumn(aStart.m_iLine);
		if (aEnd.m_iColumn >= n)
			RemoveGlyphsFromLine(aStart.m_iLine, start); // from start to end of line
		else
			RemoveGlyphsFromLine(aStart.m_iLine, start, end);
	}
	else
	{
		RemoveGlyphsFromLine(aStart.m_iLine, start); // from start to end of line
		RemoveGlyphsFromLine(aEnd.m_iLine, 0, end);
		auto& firstLine = m_Lines[aStart.m_iLine];
		auto& lastLine = m_Lines[aEnd.m_iLine];

		if (aStart.m_iLine < aEnd.m_iLine)
		{
			AddGlyphsToLine(aStart.m_iLine, firstLine.size(), lastLine.begin(), lastLine.end());
			for (int c = 0; c <= m_State.m_iCurrentCursor; c++) // move up cursors in line that is being moved up
			{
				...
				if (m_State.m_Cursors[c].GetSelectionStart() == aStart && m_State.m_Cursors[c].GetSelectionEnd() == aEnd)
					continue;
				if (m_State.m_Cursors[c].m_InteractiveEnd.m_iLine > aEnd.m_iLine)
					break;
				else if (m_State.m_Cursors[c].m_InteractiveEnd.m_iLine != aEnd.m_iLine)
					continue;
				...
				auto targetEndCoords = Coordinates(aStart.m_iLine, GetCharacterColumn(aStart.m_iLine, otherCursorNewEndCharIndex));
				auto targetStartCoords = Coordinates(aStart.m_iLine, GetCharacterColumn(aStart.m_iLine, otherCursorNewStartCharIndex));
				SetCursorPosition(targetStartCoords, c, true);
				SetCursorPosition(targetEndCoords, c, false);
			}
			RemoveLines(aStart.m_iLine + 1, aEnd.m_iLine + 1);
		}
	}
}
...
void CTextEditor::RemoveGlyphsFromLine(int aLine, int aStartChar, int aEndChar)
{
...
void CTextEditor::Render(bool aParentIsFocused)
{
	...
	m_fTextStart = m_iLeftMargin;
	static char lineNumberBuffer[16];
	if (m_bShowLineNumbers)
	{
		snprintf(lineNumberBuffer, 16, " %d ", m_Lines.size());
...
void CTextEditor::MergeCursorsIfPossible()
{
	...
	m_State.m_iCurrentCursor -= cursorsToDelete.size();
}
